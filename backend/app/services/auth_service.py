...
class AuthService:
    ...
    async def register(self, email: str, password: str, display_name: str, device_info: str | None = None, ip_address: str | None = None) -> tuple[dict, str, str]:
        ...
        refresh = self._refresh_document(user["id"], raw_refresh, device_info, ip_address, now)
        try:
        ...
        return user, create_access_token(user["id"], extra_claims={"sid": refresh["id"]}), raw_refresh

    async def login(self, email: str, password: str, device_info: str | None = None, ip_address: str | None = None) -> tuple[dict, str, str]:
        ...
        raw_refresh = generate_refresh_token()
        refresh = self._refresh_document(user["id"], raw_refresh, device_info, ip_address)
        await self.db.refresh_tokens.insert_one(refresh)
        return user, create_access_token(user["id"], extra_claims={"sid": refresh["id"]}), raw_refresh

    async def refresh_tokens(self, raw_refresh_token: str, device_info: str | None = None, ip_address: str | None = None) -> tuple[str, str]:
        now = datetime.now(timezone.utc)
        ...
        replacement = self._refresh_document(token["user_id"], raw_replacement, device_info, ip_address, now)
        claimed = await self.db.refresh_tokens.find_one_and_update(
            {"id": token["id"], "revoked_at": None},
            {"$set": {"revoked_at": now, "replaced_by": replacement["id"], "last_activity": now}},
        ...
        return create_access_token(token["user_id"], extra_claims={"sid": replacement["id"]}), raw_replacement
    ...
    def _refresh_document(self, user_id: str, raw_token: str, device_info: str | None, ip_address: str | None = None, now: datetime | None = None) -> dict:
        created_at = now or datetime.now(timezone.utc)
        return {
            "id": str(uuid.uuid4()), "user_id": user_id, "token_hash": hash_refresh_token(raw_token),
            "device_info": device_info, "ip_address": ip_address, "created_at": created_at,
